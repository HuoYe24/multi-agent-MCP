...
from ecommerce.tools import find_order_id, infer_priority, infer_ticket_category
...
def _format_order_response(order: dict) -> str:
    ...
    return (
        f"我查到订单 `{order.get('order_id')}` 的当前状态是：{order.get('status_note')}\n\n"
        f"物流公司：{order.get('carrier')}\n"
        f"运单号：{order.get('tracking_number')}\n"
        f"预计送达：{order.get('estimated_delivery')}\n\n"
        f"商品：\n{item_lines if item_lines else '- 暂无商品明细'}"
    )


def order_query_agent(state: State, mcp_client=None, working_memory=None):
    last_message = state["messages"][-1]
    ...
    if not order_id:
        return {
            "messages": [
                AIMessage(content="请提供订单号，例如 `ORD-20260510-001`，我才能帮你查询物流和订单状态。")
            ]
        }

    user_id = state.get("user_id", "anonymous")
    result = mcp_client.call_tool("order_query", {"order_id": order_id, "user_id": user_id}) if mcp_client else {}
    ...
    review = review_customer_service_response(_format_order_response(order))
    return {"messages": [AIMessage(content=review["sanitized_content"])]}


def _format_ticket_response(ticket: dict) -> str:
    priority_label = {
        "low": "普通",
        "medium": "中等",
        "high": "高",
        "urgent": "紧急",
    }.get(ticket.get("priority"), ticket.get("priority", "中等"))
    status_label = {
        "created": "已创建",
        "processing": "处理中",
        "resolved": "已解决",
        "closed": "已关闭",
    }.get(ticket.get("status"), ticket.get("status", "已创建"))
    return (
        f"已为你记录售后工单。\n\n"
        f"工单号：`{ticket.get('ticket_id')}`\n"
        f"类型：{ticket.get('category')}\n"
        f"优先级：{priority_label}\n"
        f"状态：{status_label}\n"
        f"摘要：{ticket.get('summary')}\n\n"
        f"客服会根据工单继续处理，请保留工单号方便后续查询。"
    )


def ticket_agent(state: State, mcp_client=None, working_memory=None):
    last_message = state["messages"][-1]
    message = str(last_message.content or "")
    user_id = state.get("user_id", "anonymous")
    ...
    if ticket_id:
        result = mcp_client.call_tool("ticket_query", {"ticket_id": ticket_id}) if mcp_client else {}
        if not result.get("success"):
            return {"messages": [AIMessage(content=f"工单查询失败：{result.get('error', '工具暂不可用')}")]}
        ticket = result.get("result", {})
        if ticket.get("found") is False:
            return {"messages": [AIMessage(content=f"没有查到工单 `{ticket_id}`，请确认工单号是否正确。")]}
        return {"messages": [AIMessage(content=_format_ticket_response(ticket))]}
    ...
    result = mcp_client.call_tool(
        ...
        {
            "user_id": user_id,
            "category": infer_ticket_category(details),
            "priority": infer_priority(details),
            "summary": details[:80],
            "details": details,
        },
    ) if mcp_client else {}
    ...
    ticket = result.get("result", {})
    if working_memory:
        working_memory.update(user_id, {"last_ticket_id": ticket.get("ticket_id"), "last_route": "ticket_support"})

    review = review_customer_service_response(_format_ticket_response(ticket))
    return {"messages": [AIMessage(content=review["sanitized_content"])]}


def compliance_agent(state: State, llm=None, mcp_client=None):
    last_message = state["messages"][-1]
    message = str(last_message.content or "")
    review = review_customer_service_response(message)
    risk = mcp_client.call_tool("risk_check", {"action": "customer_message_review", "user_id": state.get("user_id", "anonymous")}) if mcp_client else {}

    if not review["passed"]:
        content = (
            "这类表达可能包含不合规承诺或隐私风险，我不能按原样处理。\n\n"
            "我可以继续帮你按平台规则查询订单、创建售后工单，或根据知识库解释退换货政策。"
        )
    elif risk.get("success") and risk.get("result", {}).get("requires_manual_review"):
        content = "这个请求需要人工客服复核。我可以先为你创建售后工单，方便后续跟进。"
    else:
        content = (
            "我会按平台客服规范处理：不泄露隐私信息，不承诺超出规则的退款或赔偿，"
            "并在需要时转交人工客服复核。"
        )
