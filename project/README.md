...
An **e-commerce Multi-Agent MCP customer service system** built on the original Agentic RAG framework. It keeps **LangGraph**, **parent–child chunking**, and **hybrid dense + sparse retrieval**, and adds Redis short-term memory, an MCP tool server, OpenTelemetry tracing, and customer-service agents for order and ticket workflows.
...
Prepare environment variables before running:
...
- **LangGraph Multi-Agent Flow**: Routes e-commerce requests to direct chat, order lookup, ticket support, compliance checks, or knowledge-base RAG
- **MCP Tool Server**: Provides order query, ticket create/query, and risk-check tools over JSON-RPC style `tools/list` and `tools/call`
- **Redis Short-term Memory**: Stores short-lived chat and ticket state with JSON fallback when Redis is unavailable
- **OpenTelemetry**: Exports app and MCP server traces to Jaeger
...
| File | Purpose |
|------|---------|
| `project/app.py` | Application entry point, launches the FastAPI + Uvicorn app |
...
| `project/core/observability.py` | Optional OpenTelemetry tracing and FastAPI instrumentation |
...
⚠️ **Important:** Changing embeddings requires re-indexing all documents through the FastAPI web UI.
...
Upload documents again through the FastAPI web UI to apply new chunking.
...
Optional tracing with OpenTelemetry captures FastAPI requests, chat streaming, MCP client calls, and MCP tool server calls. Docker Compose sends traces to Jaeger.
...
export OTEL_SERVICE_NAME=multi-agent-mcp-app
export OTEL_EXPORTER_OTLP_ENDPOINT=http://localhost:4318/v1/traces
```

When running through Docker Compose, Jaeger is available at `http://127.0.0.1:16686`.

### What gets traced

| Component | Traced operations |
|-----------|-------------------|
| FastAPI | App and MCP server requests |
| Chat | Streaming request span with user and chat identifiers |
| MCP client | `tools/list` and `tools/call` requests |
| MCP server | JSON-RPC method handling |
| RAG tools | Existing LangGraph tool events remain visible in app logs/UI cards |
...
**Location:** `project/ui/fastapi_ui.py`
